...
class Guru extends CI_Controller
{
	...
	public function add()
	{
		$data = [
			...
			'keterampilan'	=> $this->master->getAllKeterampilan()
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
		...
		if ($this->form_validation->run() == FALSE) {
			$data = [
				...
				'errors'	=> [
					...
					'keterampilan' => form_error('keterampilan'),
				]
			];
			$this->output_json($data); //validasi gagal
		} else {
			$input = [
				'nip'			=> $nip,
				'nama_guru' 	=> $nama_guru,
	...
	public function do_import()
	{
		...
		foreach ($input as $d) {
			$data[] = [
				...
				'email' => $d->email,
				'keterampilan_id' => $d->keterampilan_id
			];
		}
